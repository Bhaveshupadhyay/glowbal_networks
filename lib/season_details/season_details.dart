...
import 'package:zeus/main_screens/episode/episode_detail.dart';
import 'package:zeus/main_screens/episode/landscape_video.dart';
import 'package:zeus/main_screens/episode/video_state.dart';
import 'package:zeus/modal_class/video_modal.dart';
import 'package:zeus/season_details/season_cubit.dart';
...
class SeasonDetails extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MultiBlocProvider(
      providers: [
        BlocProvider(create: (create)=>AppbarCubit()..onScroll(_scrollController)),
        BlocProvider(create: (create)=>SeasonCubit()..seasonCount(videoModal.id)),
      ...
      child: BlocListener<VideoOrientationCubit,Orientation>(
        listener: (BuildContext context, orientation) {
          if(orientation== Orientation.landscape){
            context.read<VideoCubit>().initVideoPlayer(videoModal.video);
          }
        },
        child: BlocBuilder<VideoOrientationCubit,Orientation>(
          builder: (BuildContext context, orientation) {
            if(orientation== Orientation.landscape){
              return const LandscapeVideo(isTrailer: true,);
            }
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Container(
                                    ...
                                    child: TextButton.icon(
                                      icon: Icon(Icons.play_arrow_rounded,
                                        color: Colors.white,size: 25.sp,),
                                      onPressed: (){
                                        context.read<EpisodesCubit>().playFirstEpisode();
                                      },
                                  ...
                                  BlocListener<SeasonCubit,SeasonState>(
                                    listener: (BuildContext context, SeasonState state) {
                                      if(state is SeasonListLoaded){
                                        // print(state);
                                        context.read<EpisodesCubit>().loadEpisodes(state.selectedValue, videoModal.id,videoModal.title);
                                      }
                                    },
                                        ...
                                        builder: (context,state){
                                          if(state is SeasonLoading){
                                            ...
                                            return DropdownButton<String>(
                                              ...
                                              style: Theme.of(context).textTheme.titleMedium,
                                              items: state.seasonList.map((String value) {
                                              ...
                                              onChanged: (value) {
                                                if(state.selectedValue != value) {
                                                  context.read<SeasonCubit>().seasonChanged(value!,videoModal.id);
                                                }
                                              },
                                            );
                                          }
                        ...
                        BlocListener<EpisodesCubit,EpisodesState>(
                          listener: (BuildContext context, state) {
                            if(state is EpisodesLoaded){
                              if(state.isPlay){
                                Navigator.push(context,MaterialPageRoute(builder: (context)=> EpisodeDetail(
                                  episodeModal: state.episodesList[0], seasonNumber: state.seasonNumber,seasonTitle: videoModal.title,))
                                );
                              }
                            }
                          },
                                  ...
                                  return SliverList(
                                    delegate: SliverChildBuilderDelegate(
                                        ...
                                        return Padding(
                                          ...
                                          child: InkWell(
                                              onTap: ()=>Navigator.push(context,MaterialPageRoute(builder: (context)=> EpisodeDetail(
                                                episodeModal: state.episodesList[index], seasonNumber: state.seasonNumber,seasonTitle: videoModal.title,))
                                              ),
                                              child: SearchBlock(episodeModal: state.episodesList[index],)),
                                        );
                                      },
                                      childCount: state.episodesList.length, // Number of items in the list
                                    ),
                                  );
                                }
